	...
	concurrency::parallel_for(int(0), m_Materials.GetSize(), [&](int iMaterial)
		{
			...
			for (int x = iMaterial + 1; x < m_Materials.GetSize(); x++)
			{
				...
				if (pCur->pTextureBump == pMaterial->pTextureBump)
					pCur->pTextureBump = NULL;

				if (pCur->pTextureHeight == pMaterial->pTextureHeight)
					pCur->pTextureHeight = NULL;
			}

			SAFE_RELEASE(pMaterial->pTextureAlbedo);
			SAFE_RELEASE(pMaterial->pTextureBump);
			SAFE_RELEASE(pMaterial->pTextureHeight);
        ...
        if (pMaterial->strTextureAlbedo[0]) // Check if there is an albedo texture defined.
        {
            ...
            for (int x = 0; x < iMaterial; x++)
            {
                ...
                if (0 == strcmp(pCur->strTextureAlbedo, pMaterial->strTextureAlbedo)) // If found, reuse the texture.
                {
					...
					pMaterial->pTextureHeight = pCur->pTextureHeight;
            ...
            if (!bFound)
            {
				...
				Print("Loading bump texture: %s", pMaterial->strTextureBump); // Log texture loading.
				hr = D3DXCreateTextureFromFile(pd3dDevice, pMaterial->strTextureBump, &(pMaterial->pTextureBump)); // Load texture into Direct3D.
				ASSERT(SUCCEEDED(hr), "Can't load texture from OBJ: %s", pMaterial->strTextureBump); // Check for success and assert if failed.

				Print("Loading height texture: %s", pMaterial->strTextureHeight); // Log texture loading.
				hr = D3DXCreateTextureFromFile(pd3dDevice, pMaterial->strTextureHeight, &(pMaterial->pTextureHeight)); // Load texture into Direct3D.
				ASSERT(SUCCEEDED(hr), "Can't load texture from OBJ: %s", pMaterial->strTextureHeight); // Check for success and assert if failed.
            }
        }
    }
...
HRESULT CMeshLoader::LoadMaterialsFromMTL(const CHAR* strFilePath, const CHAR* strFileName)
{
	...
	for (;;)
	{
		...
		else if (0 == strcmp(strCommand, "map_Height"))
		{
			// Texture
			InFile >> pMaterial->strTextureHeight;
		}
		else
		{
			// Unimplemented or unrecognized command
...
void CMeshLoader::InitMaterial(Material* pMaterial)
{
	...
	pMaterial->pTextureBump = NULL;
	pMaterial->pTextureHeight = NULL;
}
